...
```{r Load in the necessary packages, message=FALSE, warning=FALSE}
...
```{r Read in the Cell Ranger data, warning=FALSE}
...
```
Deleting the percent.mito/ribo objects & creating the scatter plots
```{r}
remove(percent.mito)
remove(percent.ribo)
remove(mtgene)
...
```
Cutting off over or under expressed genes and cells with the cutoff values based
on the scatter plots made earlier
```{r}
...
ElbowPlot(back)
```
The elbow plot above tells that a cutoff around 10 PC would be the most
suitable. After some trying a cutoff of 11 ended up producing the best result.
